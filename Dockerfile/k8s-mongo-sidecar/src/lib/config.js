...
var getMongoNodePortServiceLabels = function () {
    return process.env.MONGO_NODEPORT_SERVICE_LABLES || false;
}

var getMongoNodePortSeviceCollection = function() {
    if (!getMongoNodePortServiceLabels()) {
        return false;
    }
    var labels = process.env.MONGO_NODEPORT_SERVICE_LABLES.split(',');
    for (var i in labels) {
        var keyAndValue = labels[i].split('=');
        labels[i] = {
            key: keyAndValue[0],
    ...
    console.log('[MyLog][config] noderport secvice ', labels);
    return labels;
};

var getClusterExternalIP = function () {
    return process.env.CLUSTER_EXTERNAL_IP || false;
}
...
module.exports = {
  ...
  mongoNodePortServiceLabels: getMongoNodePortServiceLabels(),
  mongoNodePortSeviceCollection: getMongoNodePortSeviceCollection(),
