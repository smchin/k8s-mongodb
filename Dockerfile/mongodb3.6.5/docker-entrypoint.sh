...
if [[ "$originalArgOne" == mongo* ]] && [ "$(id -u)" = '0' ]; then
	...
	# ======copy keyfile
    [[ -d /srv/mongodb ]] || mkdir -p /srv/mongodb
...
_mongod_hack_ensure_arg() {
	echo "[somin] _mongod_hack_ensure_arg()"
...
_mongod_hack_ensure_no_arg() {
	echo "[somin] _mongod_hack_ensure_no_arg()"
...
_mongod_hack_ensure_no_arg_val() {
	echo "[somin] _mongod_hack_ensure_no_arg_val()"
	local ensureNoArg="$1"; shift
	mongodHackedArgs=()
	while [ "$#" -gt 0 ]; do
		local arg="$1"; shift
...
_mongod_hack_ensure_arg_val() {
	echo "[somin] _mongod_hack_ensure_arg_val()"
	local ensureArg="$1"; shift
...
echo "[somin] before mongod: $originalArgOne"
if [ "$originalArgOne" = 'mongod' ]; then
	echo "[somin] in mongod"
	...
	echo "[somin] shouldPerformInitdb here"
	# check for a few known paths (to determine whether we've already initialized and should thus skip our initdb scripts)
	if [ -n "$shouldPerformInitdb" ]; then
		dbPath="$(_dbPath "$@")"
		echo "[somin] shouldPerformInitdb dbPath: $dbPath"
		for path in \
			"$dbPath/WiredTiger" \
			"$dbPath/journal" \
			"$dbPath/local.0" \
	...
	echo "[somin] shouldPerformInitdb: $shouldPerformInitdb: 1620"
	if [ -n "$shouldPerformInitdb" ]; then
		mongodHackedArgs=( "$@" )
		echo "[somin] mongodHackedArgs: $mongodHackedArgs"
		if _parse_config "$@"; then
			_mongod_hack_ensure_arg_val --config "$tempConfigFile" "${mongodHackedArgs[@]}"
			echo "[somin] tempConfigFile: $tempConfigFile"
		fi
		...
		echo "[somin] shouldPerformInitdb: mongo --host 127.0.0.1 --port 27017 --quiet"
		...
		echo "[somin] shouldPerformInitdb: MONGO_INITDB_ROOT_USERNAME"
		if [ "$MONGO_INITDB_ROOT_USERNAME" ] && [ "$MONGO_INITDB_ROOT_PASSWORD" ]; then
		...
		echo "[somin] MONGO_INITDB_DATABASE: $MONGO_INITDB_DATABASE"
		echo
		for f in /docker-entrypoint-initdb.d/*; do
			case "$f" in
